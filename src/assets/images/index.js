import AosShareIcon from '~src/assets/images/icons/aos-share-icon.svg';
import ArrowDownIcon from '~src/assets/images/icons/arrow-down-icon.svg';
import ArrowRightIcon from '~src/assets/images/icons/arrow-right-icon.svg';
import ArrowUpIcon from '~src/assets/images/icons/arrow-up-icon.svg';
import BackIcon from '~src/assets/images/icons/back-icon.svg';
import CameraIcon from '~src/assets/images/icons/camera-icon.svg';
import CenterAlignIcon from '~src/assets/images/icons/center-align-icon.svg';
import CloseIcon from '~src/assets/images/icons/close-icon.svg';
import CreateNoteIcon from '~src/assets/images/icons/create-note-icon.svg';
import CreateTaskIcon from '~src/assets/images/icons/create-task-icon.svg';
import DeleteIcon from '~src/assets/images/icons/delete-icon.svg';
import FontIcon from '~src/assets/images/icons/font-icon.svg';
import IosShareIcon from '~src/assets/images/icons/ios-share-icon.svg';
import LeftAlignIcon from '~src/assets/images/icons/left-align-icon.svg';
import NavNoteIcon from '~src/assets/images/icons/nav-note-icon.svg';
import NavPlusIcon from '~src/assets/images/icons/nav-plus-button.svg';
import NavTaskIcon from '~src/assets/images/icons/nav-task-icon.svg';
import NoNoteIcon from '~src/assets/images/icons/no-note-icon.svg';
import PaddingLeftIcon from '~src/assets/images/icons/padding-left-icon.svg';
import PaddingRightIcon from '~src/assets/images/icons/padding-right-icon.svg';
import PenIcon from '~src/assets/images/icons/pen-icon.svg';
import PhotoLibraryIcon from '~src/assets/images/icons/photo-library-icon.svg';
import PlusIcon from '~src/assets/images/icons/plus-icon.svg';
import RightAlignIcon from '~src/assets/images/icons/right-align-icon.svg';
import SettingIcon from '~src/assets/images/icons/setting-icon.svg';
import TickIcon from '~src/assets/images/icons/tick-icon.svg';
import UnTickIcon from '~src/assets/images/icons/untick-icon.svg';
import SplashIcon from '~src/assets/images/splash/splash-icon.svg';
export {
  SplashIcon,
  NavPlusIcon,
  NavNoteIcon,
  NavTaskIcon,
  SettingIcon,
  CreateTaskIcon,
  CreateNoteIcon,
  PlusIcon,
  TickIcon,
  UnTickIcon,
  ArrowRightIcon,
  ArrowUpIcon,
  ArrowDownIcon,
  BackIcon,
  PhotoLibraryIcon,
  CameraIcon,
  PenIcon,
  AosShareIcon,
  IosShareIcon,
  FontIcon,
  CloseIcon,
  LeftAlignIcon,
  RightAlignIcon,
  CenterAlignIcon,
  PaddingRightIcon,
  PaddingLeftIcon,
  DeleteIcon,
  NoNoteIcon,
};
